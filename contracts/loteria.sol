...
contract loteria is ERC20, Ownable {
    ...
    // Generacion de nuevos Tokensm ERC-20
    ...
    function compraTokens(uint256 _numTokens) public payable {
        ...
        payable(msg.sender).transfer(returnValue); // .Transfer es para enviar ethers, funcion interna
        //Envio de los tokens al cliente/usuario
        _transfer(address(this), msg.sender, _numTokens); // _transfer es para enviar tokens
    }

    // Devolucion de tokens al Smart Contract
    function devolverTokens(uint _numTokens) public payable {
        // El nuermo de tokens debe er mayor a 0
        require(_numTokens > 0, "Necesitas devolver un numero de tokens mayor a 0");
        // El usuario debe acreditar los tokens que quiere devolver
        require(_numTokens <= balanceTokens(msg.sender), "No tienes los tokes que deseas devolver");
        // El usuario transifere los tokens al Smart Contract
    ...
    function compraBoleto(uint _numBoletos) public {
        ...
        require(precioTotal <= balanceTokens(msg.sender), "No tienes los tokens suficientes");
        ...
        for (uint i=0; i<_numBoletos; i++){
            ...
            // Asignacion del ADN del boleto para generacion de un ganador
            ADNBoleto[random] = msg.sender;
            // Creacion de un nuevo NFT para un boleyo
    ...
    function generadorGanador() public onlyOwner {
        ...
        // Eleccion aleatorio de un numero entre: [0:longitud]
